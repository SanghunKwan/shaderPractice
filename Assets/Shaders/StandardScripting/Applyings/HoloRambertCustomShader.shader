Shader "Applying/HoloRambertCustomShader"
{
    Properties
    {
        [Header(Main Map)]
    ...
    SubShader
    {
        ...
        CGPROGRAM
        #pragma surface surf LambertCustom noambient alpha:fade
        #pragma target 3.0
        ...
        float4 LightingLambertCustom(SurfaceOutput s, float3 lightDir, float atten)
        {
            float4 final;
            float NdotL = saturate(dot(s.Normal, lightDir));
            
            final.rgb =  lerp( s.Albedo, _LimColor.rgb, _MixingRatio) + s.Albedo * NdotL * _LightColor0.rgb * atten;
            final.a = s.Alpha;
            return final;
        }
